...
class HentaiController {
  ...
  static async create(req: Request, res: Response) {
    ...
    // Upload de Imagens
    const imagesHentai = req.files as Express.Multer.File[];
    ...
    if (!imagesHentai || imagesHentai.length === 0) {
    ...
    imagesHentai.forEach((imageHentai: Express.Multer.File) => {
      let image = "";

      // Se tiver key (S3), usa ela
      if ("key" in imageHentai && typeof imageHentai.key === "string") {
        image = imageHentai.key;
      }

      // Se estiver rodando localmente e tiver filename
      if (
        "filename" in imageHentai &&
        typeof imageHentai.filename === "string"
      ) {
        image = imageHentai.filename;
      }

      hentai.images.push(image);
    });
  ...
  static async removeHentaiById(req: Request, res: Response) {
    ...
    if (
      (hentai.user as { _id: ObjectId })._id.toString() !== user._id.toString()
    ) {
      res.status(401).json({
  ...
  static async updateHentai(req: Request, res: Response) {
    const { id } = req.params;
    const { title, description, mangaka, tags, status, format } = req.body as {
      title: string;
      description: string;
      mangaka: string;
      tags: string;
